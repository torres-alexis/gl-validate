...
required.add_argument("-a", "--assay", help = f"Specify the assay type. Available options are: {', '.join(assay_prefixes.keys())}.", action = "store", type = str)
...
multiqc_output_prefix = "_".join(["raw_multiqc", assay_prefixes[args.assay]])
...
def main():
    ...
    if args.assay not in assay_prefixes.keys():
        print_notification(f"Argument --assay must be one of the following: {', '.join(assay_prefixes.keys())}.")
        sys.exit(1)
